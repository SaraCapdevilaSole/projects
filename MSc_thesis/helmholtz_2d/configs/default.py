...
def get_config():
    ...
    wandb.name = 'hemlholtz_2d_pink_noise_corrected'
    wandb.tag = None

    # Data-related settings
    config.data = config_data = ml_collections.ConfigDict()
    config_data.dt = 1e-3  # Temporal resolution of data
    config_data.tr = 1e-2  # Downsampling rate
    config_data.T = 1  # Total time duration to use (in seconds)
    config_data.t_avoid = 0  # Time to avoid at the beginning
    config_data.parcellations_to_use = -1  # Use all voxels (-1 for full data)
    config_data.use_every_voxel = 10  # Downsample the voxel mesh (e.g. x = 10, uses 1 every 10 voxels)
    ...
    config.nondim = False # NotImplemented
    config_data.permute_data = False  

    # Loss mode
    config.surface_loss = False # NotImplemented
    ...
    optim.decay_rate = 0.9  # Learning rate decay factor
    optim.decay_steps = 2000  # Steps after which to apply decay
    optim.grad_accum_steps = 0  # Gradient accumulation (0 means none)
    ...
    config.input_dim = 3 # e.g. 3D (time, x, y)
