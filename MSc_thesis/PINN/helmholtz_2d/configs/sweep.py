...
def get_config():
    ...
    sampling_dt = 1e-4 # temporal resolution of saved data
    config_data.dt = sampling_dt 
    config_data.tr = config_data.dt # from dt -> tr 
    config_data.T = 0.01 #1 #30 # use T seconds (total = 600 - 20)
    config_data.t_avoid = 0 #0.3#0
    config_data.parcellations_to_use = -1 #5125 #645 # 838
    config_data.use_every_voxel = 1 #1 #10 # use every x voxel, e.g. with 1k mesh, use every 10 creates downsampled 100 voxel mesh
