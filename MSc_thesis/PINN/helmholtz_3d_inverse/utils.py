...
# next np.arange(min_f, max_f, step) -> determine with model

# def find_bands(bands_names, max_frequency, bands_mean=bands_mean):
#     return {name: bands_mean[name] for name in bands_names if bands_mean[name] <= max_frequency}
# @jax.jit
# def find_frequency_bands(min_f, max_f, step_f, mult_f, max_steps=10):
#     f_arr = jnp.full(max_steps, min_f)
#     def cond(arg):
#         f, i, max_f, step_f, mult_f, f_arr = arg
#         return (f <= max_f) & (i < max_steps)

#     def body(arg):
#         f, i, max_f, step_f, mult_f, f_arr = arg
#         step_f *= mult_f
#         f += step_f
#         f_arr = f_arr.at[i].set(f)
#         return (f, i + 1, max_f, step_f, mult_f, f_arr)

#     _, _, _, _, _, f_arr = lax.while_loop(cond_fun=cond, body_fun=body, init_val=(min_f, 0, max_f, step_f, mult_f, f_arr))
#     return jnp.array(f_arr)
...
def find_frequency_bands(min_f, max_f, step_f, mult_f, max_steps=20):
    ...
    return final_f_arr
# print(find_frequency_bands(50,0,0,0))

# @jit
# def find_frequency_bands(min_f, max_f, step_f, mult_f):
#     """Generate frequency bands with a step size that doubles each time."""
#     freqs = []
#     f = min_f
#     while f <= max_f:
#         freqs.append(f)
...
def load_res_mesh_coords(data_path): 
    ...
    nn_coords = jnp.array(data['nn_coords']) # data_size, 3
    res_coords = jnp.array(data['region']) # nodes in mesh precision, 3
...
def load_real_data(cd, sf=1000): 
    ...
    u_ref = jnp.array(smooth_data(data['data'], cd.sigma_gauss)) # T, p
    # coords = jnp.array(data['locs']) # p, 3
    nn_coords, res_coords = load_res_mesh_coords(cd.res_data_path)
    Qs = None # T, p
    t_star = jnp.linspace(0, data['data'].shape[0]/sf, data['data'].shape[0]) # T
...
def load_mesh_info(cd):
    curv = np.loadtxt(cd.curv_path, delimiter=',')
    normals = np.loadtxt(cd.norm_path, delimiter=',')
    curv = jnp.array(curv) # (p,)
    normals = jnp.array(normals) # (p, 3)
    # for a sphere: normals = coords / jnp.linalg.norm(coords, axis=1, keepdims=True)
    # for a unit sphere: normals = coords
    # H = 1/2 * (kappa_1 + kappa_2) = 1 / r
    return curv, normals
...
def get_dataset(config_dataset):
    ...
    # Qs = load_Qs_noise(config_dataset, permutted_data[1].shape[1])
    res_data = res_data if res_data is not None else data[2] # pass non-downsampled coords
    return *processed_data, res_data #t_star, u_ref, coords, Qs, curv, normals
...
config = get_config()
print(get_dataset(config.data)[-1].shape)
# print(jnp.mean(Qs))
